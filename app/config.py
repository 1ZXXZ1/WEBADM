...
"for the API server process user."
    ...
    @field_validator("POLZA_AI_KEY", mode="after")
    @classmethod
    def _validate_polza_ai_key(cls, v: str) -> str:
        """Warn at startup if POLZA_AI_KEY is empty — AI won't work without it.

        v2.0.4: Previously, an empty key was silently accepted, and AI requests
        would only fail at runtime with a confusing 401 error. Now we log a
        clear WARNING at startup so the admin knows to set the key.
        """
        if not v or not v.strip():
            logger.warning(
                "[CONFIG] SAMBA_POLZA_AI_KEY is empty — AI assistant will NOT work. "
                "Set SAMBA_POLZA_AI_KEY in .env to a valid Polza.ai API key. "
                "Get a key at: https://polza.ai/dashboard → API Keys"
            )
        elif v.startswith("pza_") and len(v) < 20:
            # Suspiciously short key — likely truncated or placeholder
            logger.warning(
                "[CONFIG] SAMBA_POLZA_AI_KEY looks too short (%d chars) — "
                "might be truncated or a placeholder. Valid keys are usually 30+ chars. "
                "Current key starts with 'pza_'",
                len(v),
            )
        return v
    ...
    model_config = {
        "env_prefix": "SAMBA_",
        # v2.0.4 fix: Include /etc/webadc/.env as fallback path.
        # Previously only ".env" (relative to CWD) was listed, which means
        # pydantic-settings could NOT find the file when the service runs
        # via systemd (CWD = /, not /etc/webadc/).  This caused
        # WEB_ENABLED=false in /etc/webadc/.env to be silently ignored,
        # defaulting to True.  Adding the absolute production path ensures
        # the settings model always reads from the correct .env file.
        "env_file": [".env", "/etc/webadc/.env"],
