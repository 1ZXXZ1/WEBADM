...
Примеры::
    ...
    curl -H 'Authorization: Bearer <jwt_token>' https://localhost:8099/api/v1/users/

    # Использование с curl (API-ключ):
    curl -H 'X-API-Key: <api_key>' https://localhost:8099/api/v1/users/
"""
...
DEFAULT_SERVER = "https://127.0.0.1:8099"
TOKEN_FILE = Path.home() / ".ds_auth_token"
ENV_SERVER = "SAMBA_API_SERVER"
ENV_API_KEY = "SAMBA_API_KEY"
# v3.3.3 — Self-signed certificates are common in internal deployments.
# Default to NOT verifying SSL, but allow override via SAMBA_API_VERIFY_SSL=1.
# This matches the behavior of `curl -k`.
ENV_VERIFY_SSL = "SAMBA_API_VERIFY_SSL"


def _verify_ssl_default() -> bool:
    """Return True if SSL certificates should be verified.

    Default: False (skip verification — for self-signed certs in
    internal deployments). Override via SAMBA_API_VERIFY_SSL=1.
    """
    return os.environ.get(ENV_VERIFY_SSL, "0").lower() in ("1", "true", "yes")
...
class AuthClient:
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        # v3.3.3 — SSL verification. Default False (skip) for self-signed
        # certificates in internal deployments. Override per-request via
        # kwargs['verify'] or globally via SAMBA_API_VERIFY_SSL env var.
        if "verify" not in kwargs:
            kwargs["verify"] = _verify_ssl_default()

        try:
            resp = _requests.request(method, url, headers=headers, timeout=30, **kwargs)
        except _requests.ConnectionError as _ce:
            # Provide more helpful error for SSL failures
            base_lower = self.base_url.lower()
            if base_lower.startswith("https://"):
                raise click.ClickException(
                    f"Не удалось подключиться к {self.base_url}.\n"
                    f"  Сервер использует HTTPS — проверьте:\n"
                    f"  1. Сервер запущен с SSL (SSL_CERTFILE/SSL_KEYFILE в .env)\n"
                    f"  2. Порт 8099 открыт: ss -tlnp | grep 8099\n"
                    f"  3. Сертификат trusted или используйте --insecure\n"
                    f"  Original error: {_ce}"
                )
...
@click.option(
    ...
    envvar=ENV_SERVER,
    show_default=True,
    help="URL сервера Samba AD API (по умолчанию HTTPS).",
)
@click.option(
    "--api-key",
    required=False,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Skip SSL certificate verification (для self-signed сертификатов).",
)
@click.option(
    "--verify/--no-verify",
    "verify_ssl",
    default=None,
    help="Проверять SSL сертификат (по умолчанию: не проверять для self-signed).",
)
@click.pass_context
def cli(ctx: click.Context, server: str, api_key: Optional[str], token: Optional[str],
        json_output: bool, insecure: bool, verify_ssl: Optional[bool]) -> None:
    ...
    \b
    SSL/HTTPS:
      По умолчанию сертификат НЕ проверяется (для self-signed в
      internal deployments). Чтобы включить проверку:
        --verify
      Или через env: SAMBA_API_VERIFY_SSL=1
    """
    # v3.3.3 — Resolve SSL verification:
    # 1. --verify / --no-verify (explicit)
    # 2. --insecure (back-compat, same as --no-verify)
    # 3. SAMBA_API_VERIFY_SSL env var
    # 4. Default: False (skip verification for self-signed)
    if verify_ssl is not None:
        os.environ[ENV_VERIFY_SSL] = "1" if verify_ssl else "0"
    elif insecure:
        os.environ[ENV_VERIFY_SSL] = "0"
...
def login_cmd(ctx: click.Context, username: str, password: str, save: bool) -> None:
    ...
    if save:
        ...
        click.echo(f"         https://localhost:8099/api/v1/users/")
        click.echo()
        ...
        access = data.get('access_token', '')
        click.echo(f"    curl -H 'Authorization: Bearer <token>' https://localhost:8099/api/v1/users/")
...
def refresh_cmd(ctx: click.Context) -> None:
    ...
    try:
        resp = _requests.post(
            ...
            timeout=30, verify=_verify_ssl_default(),
        )
    except _requests.ConnectionError:
        raise click.ClickException(f"Не удалось подключиться к серверу {client.base_url}. Проверьте, что API сервер запущен.")
    except _requests.Timeout:
...
def whoami_cmd(ctx: click.Context) -> None:
    ...
    import requests as _requests
    try:
        resp = _requests.get(f"{client.base_url}/health", timeout=5, verify=_verify_ssl_default())
        if resp.ok:
...
def key_create(ctx: click.Context, user_id: int, name: str, role: str, expires_days: Optional[int], weight: int, description: str) -> None:
    ...
    click.echo(f"    curl -H 'X-API-Key: {raw_key}' https://localhost:8099/api/v1/users/")
...
def quickkey(ctx: click.Context, username: str, password: str, key_name: str, role: str, expires_days: Optional[int]) -> None:
    ...
    try:
        users_resp = _requests.get(f"{client.base_url}/api/v1/mgmt/users", headers=headers, timeout=30, verify=_verify_ssl_default())
        if users_resp.ok:
    ...
    click.echo(f"    curl -H 'Authorization: Bearer <jwt>' https://localhost:8099/api/v1/users/")
    click.echo()
    ...
    if has_auth:
        try:
            auth_resp = _requests.get(
                ...
                params={"limit": 1},
                timeout=5, verify=_verify_ssl_default(),
            )
            if auth_resp.ok:
                click.secho(f"  Авторизация: ОК ({auth_type})", fg="green")
            else:
