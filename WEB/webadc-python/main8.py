...
from __future__ import annotations  # enables X | None syntax on Python 3.7–3.9
...
async def spa_fallback(full_path: str):
    ...
    exists = await loop.run_in_executor(None, file_path.is_file)
    if exists:
...
# ─── Custom exception handler to suppress noisy Windows disconnect errors ────

def _windows_exception_handler(loop, context):
    exception = context.get('exception')
    if isinstance(exception, ConnectionResetError):
        return  # ignore client disconnect noise on Windows
...
async def main():
    ...
    if not Path(CERT_FILE).exists() or not Path(KEY_FILE).exists():
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, generate_self_signed_cert, CERT_FILE, KEY_FILE)
    ...
    # Set up a custom exception handler to suppress Windows ConnectionResetError noise
    loop = asyncio.get_running_loop()
